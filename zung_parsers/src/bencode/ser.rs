...
impl AsRef<[u8]> for BencodeSerializer {
    fn as_ref(&self) -> &[u8] {
        self.buffer.as_ref()
    }
}
...
pub fn to_bencode_bytes<T>(value: &T) -> Result<Vec<u8>>
where
    T: Serialize,
{
    let mut serializer = BencodeSerializer { buffer: Vec::new() };
...
impl<'a> SerializeMap<'a> {
    pub fn new(ser: &'a mut BencodeSerializer, len: usize) -> SerializeMap<'_> {
        SerializeMap {
            ser,
