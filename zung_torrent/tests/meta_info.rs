...
mod getters {
    ...
    #[test]
    fn torrent_size() {
        assert_eq!(CLIENT.arch.meta_info().size(), 1001730048);
        assert_eq!(CLIENT.mit.meta_info().size(), 13669236736);
        assert_eq!(CLIENT.kali.meta_info().size(), 4102389760);
    }
...
mod calculators {
    ...
    fn info_hash_url_encode() {
        ...
        assert_eq!(
            arch,
            "%68%53%ab%2b%86%b2%cb%6a%3c%77%8b%8a%af%e3%df%fd%94%24%23%21".to_ascii_uppercase()
        );
        assert_eq!(
            mit,
            "%c5%f1%f7%e8%6c%5f%18%63%6a%4b%64%50%22%99%c3%88%0d%90%85%a8".to_ascii_uppercase()
        );
        assert_eq!(
            kali,
            "%f2%4f%4f%54%df%51%11%8b%03%f9%9c%74%41%6e%45%54%ab%88%d2%2b".to_ascii_uppercase()
        );
    }
