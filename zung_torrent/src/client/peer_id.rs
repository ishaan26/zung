...
impl PeerID {
    ...
    pub const PEERID_SIZE: u8 = std::mem::size_of::<Self>() as u8;
    ...
    pub const fn as_bytes(&self) -> [u8; Self::PEERID_SIZE as usize] {
        let bytes = self as *const Self as *const [u8; Self::PEERID_SIZE as usize];
        unsafe { *bytes }
    }
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        unsafe {
            write!(
                ...
                std::str::from_utf8_unchecked(&self.custom),
            )
        }
    }
}
...
impl TryFrom<&[u8]> for PeerID {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
        if bytes.len() != Self::PEERID_SIZE as usize {
            return Err(format!(
...
mod peer_id_tests {
    ...
    fn test_peer_id_serialization() {
        ...
        assert_eq!(&bytes[3..4], &peer_id.end);
        assert_eq!(&bytes[4..8], &peer_id.pid);
        assert_eq!(&bytes[8..20], &peer_id.custom);
    }
    ...
    fn test_to_url_encoded() {
        ...
        let expected = "%2D%5A%47%2D%31%32%33%34%35%36%37%38%39%41%42%43%44%45%46%47";
